...
class Level3:
    ...
    def _check_boss_collision(self):
        if hasattr(self, 'boss') and self.player.rect.colliderect(self.boss.rect):
            if (self.player.rect.bottom < self.boss.rect.top + 20 and
                ...
                self.player.just_attacked_boss = True
    ...
    def _render_game_objects(self, screen):
        ...
        # Рендеринг игрока с использованием текущего кадра анимации
        self._render_player(screen, self.player.image, camera_x)
    ...
    def _render_player(self, screen, image, camera_x):
        x = int((self.player.rect.x - camera_x) * self.scale_x)
